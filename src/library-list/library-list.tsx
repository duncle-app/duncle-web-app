...
function LibraryList(props: LibraryOverviewProps) {
    ...
    const [libraries, setLibraries]: [Library[], React.Dispatch<React.SetStateAction<Library[]>>] = useState(initialLibrary)
...
useEffect( () => {
    ...
    return (
        <div>
            {libraries.map((lib: Library) => <LibraryOverview name={lib.name} />)}
        </div>
    );
}
